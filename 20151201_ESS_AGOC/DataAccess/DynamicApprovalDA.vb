...
Public Class DynamicApprovalDA
            ...
            Select Case objEN.HeaderType
                Case "Train"
                    Select Case objEN.HistoryType
                        Case "RegTra"
                            objDA.strQuery = "  select T0.Code,T4.ExtEmpNo,T0.U_Z_HREmpID,T0.U_Z_HREmpName,T0.U_Z_TrainCode,T0.U_Z_CourseCode,T0.U_Z_CourseName,T0.U_Z_CourseTypeDesc,Convert(varchar(10),T0.U_Z_Startdt,103) AS U_Z_Startdt,Convert(Varchar(10),T0.U_Z_Enddt,103) AS U_Z_Enddt,"
                            ...
                            objDA.strQuery += " T6.U_Z_TrainLoc,CAST(T6.U_Z_EstExpe as decimal(10,2)) AS U_Z_EstExpe,cast(T5.U_Z_AttCost as decimal(10,2)) AS U_Z_AttCost,Case T0.U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus, T0.U_Z_CurApprover AS 'U_Z_CurApprover',T0.U_Z_NxtApprover AS 'U_Z_NxtApprover',"
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +''+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +''+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += "  from [@Z_HR_TRIN1] T0 JOIN OHEM T4 ON T0.U_Z_HREmpID = T4.empID and (T0.""U_Z_AppStatus""='P' or T0.""U_Z_AppStatus""='-') "
                            ...
                            objDA.strQuery += " And (T0.U_Z_NxtApprover = '" + objEN.UserCode + "')"
                        ...
                        Case "NewTra"
                            ...
                            objDA.strQuery += " Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus, U_Z_CurApprover AS 'U_Z_CurApprover',U_Z_NxtApprover AS 'U_Z_NxtApprover' ,"
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +''+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +''+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += " from [@Z_HR_ONTREQ] T0 JOIN OHEM T4 ON T0.U_Z_HREmpID = T4.empID and (T0.""U_Z_AppStatus""='P' or T0.""U_Z_AppStatus""='-') "
                            objDA.strQuery += " JOIN [@Z_HR_APPT1] T1 ON T4.empID = T1.U_Z_OUser "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry  "
                            ' objDA.strQuery += " And (T0.U_Z_CurApprover = '" + objEN.UserCode + "' OR T0.U_Z_NxtApprover = '" + objEN.UserCode + "')"
                ...
                Case "EmpLife"
                    Select Case objEN.HistoryType
                        Case "EmpPro"
                            ...
                            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T0.U_Z_AppRequired,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = '" + objEN.HeaderType + "' Order by Convert(Numeric,T0.Code) Desc"
                        Case "EmpPos"
                            objDA.strQuery = " select ""Code"",R3.ExtEmpNo,T0.""U_Z_EmpId"",T0.""U_Z_FirstName"",T0.U_Z_Dept,T1.""U_Z_DeptName"",""U_Z_PosCode"",""U_Z_PosName"",""U_Z_JobName"",""U_Z_OrgCode"",""U_Z_OrgName"","
                        ...
                        Case "PerObj"
                            objDA.strQuery = " select ""U_DocEntry"",R3.ExtEmpNo,T0.""U_Empid"",T0.""U_Empname"",U_DeptCode,U_PeoobjCode,U_PeoobjName,U_PeoCatDesc,CAST(isnull(""U_Weight"",0) as decimal(10,2)) AS U_Weight,U_Remarks,U_PeoCategory,"
                            objDA.strQuery += " Case U_Z_AppRequired when 'Y' then 'Yes' else 'No' End as  'U_Z_AppRequired',Convert(Varchar(10),U_Z_AppReqDate,103) AS 'U_Z_AppReqDate',CONVERT(VARCHAR(8),U_Z_ReqTime,108) AS 'U_Z_ReqTime',"
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_NxtApprover ),'') AS NextAppName,"
                            ...
                            objDA.strQuery += " And (T0.U_NxtApprover = '" + objEN.UserCode + "')"
                        ...
                        Case "PerHour"
                            ...
                            objDA.strQuery = "Select * FROM (Select Distinct T0.""Code"" as ""Code"",T4.dept as 'DeptCode' ,T0.""U_Z_EMPID"",T4.U_Z_EmpID as 'Badge No',T0.""U_Z_EMPNAME"",convert(varchar(10),""U_Z_StartDate"",103) AS ""U_Z_StartDate"","
                    ...
                    End Select
                Case "ExpCli"
                    objDA.strQuery = " Select Code,T0.U_Z_EmpID,T4.ExtEmpNo,U_Z_EmpName,Convert(Varchar(10),U_Z_SubDt,103) AS U_Z_SubDt,Convert(Varchar(10),U_Z_Claimdt,103) AS U_Z_Claimdt,U_Z_ExpType,U_Z_Currency,CAST(isnull(U_Z_CurAmt,0) as decimal(10,2)) AS  U_Z_CurAmt,U_Z_UsdAmt,U_Z_ReimAmt,isnull(U_Z_Attachment,'') AS U_Z_Attachment,Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus,"
                ...
                Case "LveReq"
                    Select Case objEN.HistoryType
                        Case "LveReq"
                            ...
                            objDA.strQuery = "Select * FROM (Select Distinct T0.""Code"" as ""Code"",T0.""U_Z_EMPID"",T4.ExtEmpNo,T0.""U_Z_EMPNAME"",""U_Z_TrnsCode"",isnull(""U_Z_LeaveName"",'') AS U_Z_LeaveName,convert(varchar(10),""U_Z_StartDate"",103) AS ""U_Z_StartDate"","
                        ...
                        Case "RetLve"
                            Dim strLvetype As String = getLeaveType(objEN.UserCode)
                            objDA.strQuery = "Select * FROM (Select Distinct T0.""Code"" as ""Code"",T0.""U_Z_EMPID"",T4.ExtEmpNo,T0.""U_Z_EMPNAME"",""U_Z_TrnsCode"",isnull(""U_Z_LeaveName"",'') AS U_Z_LeaveName,convert(varchar(10),""U_Z_StartDate"",103) AS ""U_Z_StartDate"","
                            objDA.strQuery += " convert(varchar(10),""U_Z_EndDate"",103) AS ""U_Z_EndDate"" ,cast(T0.U_Z_NoofDays as decimal(10,2)) AS ""U_Z_NoofDays"",convert(varchar(10),T0.""U_Z_RetJoiNDate"",103)  AS ""U_Z_RetJoiNDate"",""U_Z_RAppRemarks"",cast(T0.U_Z_LevBal as decimal(10,2)) AS 'U_Z_LevBal',Convert(Varchar,""U_Z_Notes"") as ""U_Z_Notes"",convert(varchar(10),"
                ...
                Case "Train"
                    Select Case objEN.HistoryType
                        Case "RegTra"
                            ...
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += " from [@Z_HR_TRIN1] T0 JOIN OHEM T4 ON T0.U_Z_HREmpID = T4.empID"
                            objDA.strQuery += " left join [@Z_HR_OTRIN] T5 on T5.U_Z_TrainCode=T0.U_Z_TrainCode"
                            objDA.strQuery += " left join [@Z_HR_ONTREQ] T6 on T6.DocEntry=T5.U_Z_NewTrainCode"
                            objDA.strQuery += " JOIN [@Z_HR_APPT1] T1 ON T4.empID = T1.U_Z_OUser "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                        ...
                        Case "NewTra"
                            objDA.strQuery = "select T0.DocEntry,Convert(Varchar(10),U_Z_ReqDate,103) AS U_Z_ReqDate,T4.ExtEmpNo,T0.U_Z_HREmpID,U_Z_HREmpName,U_Z_CourseName,U_Z_CourseDetails,convert(varchar(10),U_Z_TrainFrdt,103) as U_Z_TrainFrdt,convert(varchar(10),U_Z_TrainTodt,103) as U_Z_TrainTodt,cast(U_Z_TrainCost as decimal(10,2)) AS U_Z_TrainCost,U_Z_Notes,"
                            ...
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName "
                            objDA.strQuery += "  from [@Z_HR_ONTREQ] T0 JOIN OHEM T4 ON T0.U_Z_HREmpID = T4.empID "
                            objDA.strQuery += " JOIN [@Z_HR_APPT1] T1 ON T4.empID = T1.U_Z_OUser "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry  "
                ...
                Case "EmpLife"
                    Select Case objEN.HistoryType
                        Case "EmpPro"
                            ...
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += " From ""@Z_HR_HEM2"" T0 Join OHEM R3 on R3.""empID""=T0.""U_Z_EmpId"" JOIN [@Z_HR_APPT3] T1 ON R3.""dept"" = T1.U_Z_DeptCode "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry AND T0.""U_Z_Posting""='N'"
                            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = '" + objEN.HeaderType + "' Order by Convert(Numeric,T0.Code) Desc"
                        Case "EmpPos"
                            ...
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += "  from ""@Z_HR_HEM4"" T0 Join OHEM R3 on R3.""empID""=T0.""U_Z_EmpId"" JOIN [@Z_HR_APPT3] T1 ON R3.""dept"" = T1.U_Z_DeptCode "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry AND T0.""U_Z_Posting""='N'"
                        ...
                        Case "PerObj"
                            objDA.strQuery = " select ""U_DocEntry"",R3.ExtEmpNo,T0.""U_Empid"",T0.""U_Empname"",U_DeptCode,U_PeoobjCode,U_PeoobjName,U_PeoCatDesc,U_PeoCategory,CAST(isnull(U_Weight,0) as decimal(10,2)) AS U_Weight,U_Remarks,"
                            objDA.strQuery += " Case U_Z_AppRequired when 'Y' then 'Yes' else 'No' End as  'U_Z_AppRequired',Convert(Varchar(10),U_Z_AppReqDate,103) AS 'U_Z_AppReqDate',CONVERT(VARCHAR(8),U_Z_ReqTime,108) AS 'U_Z_ReqTime',"
                            objDA.strQuery += " Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus, U_CurApprover AS 'U_CurApprover',U_NxtApprover AS 'U_NxtApprover', "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_CurApprover ),'') AS CurAPPName, "
                            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_NxtApprover ),'') AS NextAppName"
                            objDA.strQuery += " from ""U_PEOPLEOBJ"" T0 JOIN OHEM R3 on R3.""empID""=T0.""U_Empid"" Join  [@Z_HR_APPT3] T1 ON R3.""dept"" = T1.U_Z_DeptCode "
                            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
                            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry "
                        ...
                        Case "PerHour"
                            '    Dim strEmpId As String = GetEmpId(objEN.UserCode)
                            objDA.strQuery = "Select * FROM (Select Distinct T0.""Code"" as ""Code"",T4.dept as 'DeptCode' ,T0.""U_Z_EMPID"",T4.ExtEmpNo as 'Badge No',T0.""U_Z_EMPNAME"",convert(varchar(10),""U_Z_StartDate"",103) AS ""U_Z_StartDate"","
                            objDA.strQuery += " convert(varchar(10),""U_Z_EndDate"",103) AS ""U_Z_EndDate"" ,CAST(isnull(T0.""U_Z_NoofDays"",0) as decimal(10,2)) AS ""U_Z_NoofDays"",U_Z_FromTime,""U_Z_AppRemarks"",U_Z_ToTime,Convert(Varchar,""U_Z_Notes"") as ""U_Z_Notes"","
                            objDA.strQuery += " case ""U_Z_Status"" when 'P' then 'Pending' when 'R' then 'Rejected' "
                            objDA.strQuery += " when 'A' then 'Approved' end as ""U_Z_Status"", "
                            objDA.strQuery += " Case U_Z_AppRequired when 'Y' then 'Yes' else 'No' End as  'U_Z_AppRequired',convert(varchar(10),""U_Z_AppReqDate"",103) AS U_Z_AppReqDate,CONVERT(VARCHAR(8),U_Z_ReqTime,108) AS 'U_Z_ReqTime',"
        ...
        Try
            ...
            Select Case objEN.HistoryType
                Case "RegTra", "NewTra", "Rec", "AppShort", "EmpPro", "EmpPos", "TraReq", "PerHour", "IntAppReq", "PerObj", "BankTime"
                    objDA.strQuery = " Select DocEntry,U_Z_DocEntry,U_Z_DocType,T1.ExtEmpNo AS U_Z_EmpId,U_Z_EmpName,U_Z_ApproveBy,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NextApprover ),'') AS U_Z_NextApprover,convert(varchar(10),CreateDate,103) as CreateDate ,"
                    objDA.strQuery += " LEFT(CONVERT(VARCHAR(5), CreateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), CreateTime, 9),2) AS CreateTime,convert(varchar(10),T0.UpdateDate,103) as UpdateDate,LEFT(CONVERT(VARCHAR(5), UpdateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), UpdateTime, 9),2) AS UpdateTime,"
                    objDA.strQuery += " Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus,U_Z_Remarks From [@Z_HR_APHIS] T0 JOIN OHEM T1 ON T0.U_Z_EmpId=T1.empID"
                ...
                Case "LveReq", "ExpCli", "RetLve"
                    objDA.strQuery = " Select DocEntry,U_Z_DocEntry,U_Z_DocType,T1.ExtEmpNo AS U_Z_EmpId,U_Z_EmpName,U_Z_ApproveBy,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NextApprover ),'') AS U_Z_NextApprover,"
                    objDA.strQuery += " convert(varchar(10),CreateDate,103) as CreateDate ,LEFT(CONVERT(VARCHAR(5), CreateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), CreateTime, 9),2) AS CreateTime, convert(varchar(10),T0.UpdateDate,103) as UpdateDate,"
                    objDA.strQuery += " LEFT(CONVERT(VARCHAR(5), UpdateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), UpdateTime, 9),2) AS UpdateTime,Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus,U_Z_Remarks,U_Z_Year,U_Z_Month From [@Z_HR_APHIS] T0 JOIN OHEM T1 ON T0.U_Z_EmpId=T1.empID"
                    objDA.strQuery += " Where U_Z_DocType = '" + objEN.HistoryType + "'"
                    objDA.strQuery += " And U_Z_DocEntry = '" + objEN.DocEntry + "'"
            End Select
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
        ...
        Try
            ...
            Try
                ...
                If oRecSet.RecordCount > 0 Then
                    ...
                    Try
                        objDA.strQuery = "Insert into [@Z_PAY_OLETRANS](Code,Name,U_Z_EmpId1,U_Z_EMPID,U_Z_EMPNAME,U_Z_TrnsCode,U_Z_LeaveName,U_Z_StartDate,U_Z_EndDate,U_Z_NoofDays,U_Z_Notes,U_Z_Month,U_Z_Year,U_Z_ReJoiNDate,U_Z_Cutoff,U_Z_LevBalance)"
                        ...
                        objDA.strQuery += " '" & oRecSet.Fields.Item("U_Z_Year").Value & "','" & oRecSet.Fields.Item("U_Z_ReJoiNDate").Value & "','" & CutOff & "','" & oRecSet.Fields.Item("U_Z_LevBal").Value & "')"
                        oTemp.DoQuery(objDA.strQuery)
                    Catch ex As Exception
                        objDA.strQuery = "Insert into [@Z_PAY_OLETRANS](Code,Name,U_Z_EMPID,U_Z_EMPNAME,U_Z_TrnsCode,U_Z_LeaveName,U_Z_StartDate,U_Z_EndDate,U_Z_NoofDays,U_Z_Notes,U_Z_Month,U_Z_Year,U_Z_ReJoiNDate,U_Z_Cutoff,U_Z_LevBalance)"
            ...
            If reimbused = "No" Or reimbused = "N" Then
                ...
                If oTemp.RecordCount > 0 Then
                    ...
                    If Vjov.Add <> 0 Then
                        ...
                        objDA.strQuery = "Update [@Z_HR_EXPCL] Set U_Z_AppStatus = 'P' Where Code = '" + aCode + "'"
                        oRecSet.DoQuery(objDA.strQuery)
                    Else
    ...
    Public Function validateLeave(ByVal aLeveCode As String, ByVal aLeveBal As String, ByVal aNofoDays As String, ByVal SapCompany As SAPbobsCOM.Company) As String
        ...
        objDA.objMainCompany = SapCompany
        oTemp = objDA.objMainCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim strSQL, Maxdays As String
        Maxdays = getMaxDays(aLeveCode)
        ...
        If oTemp.RecordCount > 0 Then
            If oTemp.Fields.Item("U_Z_StopProces").Value = "Y" Then
                If CDbl(aLeveBal) < CDbl(aNofoDays) Then
                    ...
                    Return objDA.strmsg
                ElseIf CDbl(Maxdays) < CDbl(aNofoDays) Then
                    objDA.strmsg = "Maximum of " & Maxdays & " days allowed for this type of leave"
                    Return objDA.strmsg
                End If
            End If
        End If
        Return "Success"
    End Function
    Public Function getMaxDays(ByVal aLeveCode As String) As String
        Dim RStatus As String
        Try
            objDA.con.Open()
            objDA.cmd = New SqlCommand("Select CAST(isnull(T0.U_Z_MaxDays,0) as decimal(10,2))  from ""@Z_PAY_LEAVE"" T0 where Code='" & aLeveCode & "'", objDA.con)
            objDA.cmd.CommandType = CommandType.Text
            RStatus = objDA.cmd.ExecuteScalar()
            objDA.con.Close()
            Return RStatus
        Catch ex As Exception
            DBConnectionDA.WriteError(ex.Message)
    ...
    Public Function GetLeaveBalance(ByVal objEN As DynamicApprovalEN) As DataSet
        Try
            objDA.strQuery = "select U_Z_LeaveCode,U_Z_LeaveName, cast(U_Z_Balance as decimal(10,2)) AS U_Z_Balance from [@Z_EMP_LEAVE_BALANCE]  where U_Z_EmpID='" & objEN.EmpId & "' and U_Z_Year=" & objEN.Year & ""
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
            objDA.sqlda.Fill(objDA.dss4)
            Return objDA.dss4
        Catch ex As Exception
