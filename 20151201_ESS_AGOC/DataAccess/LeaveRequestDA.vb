...
Public Class LeaveRequestDA
    ...
    Public Function SaveLeaveRequest(ByVal objen As LeaveRequestEN) As String
        Try
            ...
            objDA.strQuery = "Insert into ""@Z_PAY_OLETRANS1"" (""Code"",""Name"",""U_Z_EMPID"",""U_Z_EMPNAME"",""U_Z_TransType"",""U_Z_TrnsCode"",""U_Z_LeaveName"",""U_Z_StartDate"","
            objDA.strQuery += " ""U_Z_EndDate"",""U_Z_NoofDays"",""U_Z_Notes"",""U_Z_ReJoiNDate"",""U_Z_Status"",""U_Z_LevBal"",""U_Z_Year"",""U_Z_Month"",""U_Z_TotalLeave"",""U_Z_CrEmpID"",""U_Z_CrEmpName"") "
            objDA.strQuery += "  Values ('" & strCode & "','" & strCode & "','" & objen.Empid & "','" & objen.EmpName & "','L','" & objen.LeaveCode & "','" & objen.LeaveName & "',"
            objDA.strQuery += " '" & objen.FromDate.ToString("yyyy/MM/dd") & "','" & objen.ToDate.ToString("yyyy/MM/dd") & "','" & objen.NoofDays & "','" & objen.Notes & "',"
            objDA.strQuery += " '" & objen.RejoinDt.ToString("yyyy/MM/dd") & "','" & objen.Status & "'," & objen.LeaveBalance & "," & objen.Year & "," & objen.Month & ","
            objDA.strQuery += " " & objen.TotalLeave & ",'" & objen.AltEmpid & "','" & objen.AltEmpName & "')"
            objDA.cmd = New SqlCommand(objDA.strQuery, objDA.con)
            objDA.con.Open()
    ...
    Public Function PopupSearchBind(ByVal objen As LeaveRequestEN) As DataSet
        Try
            ...
            Else
                objDA.strQuery = "Select ""Code"",""Name"" from ""@Z_PAY_LEAVE"" order by ""U_Z_Sort"""
            End If
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
        ...
        Try
            ...
            If objDA.ds4.Tables(0).Rows.Count > 0 Then
                If objDA.ds4.Tables(0).Rows(0)(0).ToString() = "" Then
                    objDA.strQuery = "Select T0.""Code"",T0.""Name"",CAST(isnull(T2.U_Z_Balance,0) as decimal(10,2)) AS  'Balance'  from ""@Z_PAY_LEAVE"" T0  Join  [@Z_EMP_LEAVE_BALANCE] T2 on T2.U_Z_LeaveCode=T0.Code    And T2.U_Z_EmpID='" & objen.Empid & "' and T2.U_Z_Year=" & objen.Year & " and isnull(T0.U_Z_HideESS,'N')='N' order by T0.""U_Z_Sort"""
                Else
                    objDA.strQuery = " Select T1.""U_Z_LeaveCode"" 'Code',T0.""Name"",CAST(isnull(T2.U_Z_Balance,0) as decimal(10,2)) AS 'Balance'  from  ""@Z_PAY_OALMP"" T1  join ""@Z_PAY_LEAVE"" T0 on T0.""Code""=T1.""U_Z_LeaveCode""  Join  [@Z_EMP_LEAVE_BALANCE] T2  on T2.U_Z_LeaveCode=T0.Code and  T2.U_Z_EmpID='" & objen.Empid & "' and T2.U_Z_Year=" & objen.Year & " and isnull(T0.U_Z_HideESS,'N')='N'  where ""U_Z_Terms""='" & objDA.ds4.Tables(0).Rows(0)(0).ToString() & "' order by T0.""U_Z_Sort"""
                End If
            End If
        Catch ex As Exception
            objDA.strQuery = "Select T0.""Code"",T0.""Name"",CAST(isnull(T2.U_Z_Balance,0) as decimal(10,2)) AS 'Balance'  from ""@Z_PAY_LEAVE"" T0  Join  [@Z_EMP_LEAVE_BALANCE] T2 on T2.U_Z_LeaveCode=T0.Code   And T2.U_Z_EmpID='" & objen.Empid & "' and T2.U_Z_Year=" & objen.Year & " and isnull(T0.U_Z_HideESS,'N')='N' order by T0.""U_Z_Sort"""
    ...
    Public Function GetLeaveBalance(ByVal objen As LeaveRequestEN) As String
        objDA.con.Open()
        objDA.cmd = New SqlCommand("select cast(isnull(U_Z_Balance,0) as decimal(10,2)) from [@Z_EMP_LEAVE_BALANCE] where U_Z_Year=" & objen.Year & " and U_Z_EmpID='" & objen.Empid & "' and U_Z_LeaveCode='" & objen.LeaveCode & "'", objDA.con)
