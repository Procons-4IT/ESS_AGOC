...
Public Class LoanApprovalDA
    ...
    Public Function LoadHistory(ByVal objEN As LoanApprovalEN) As DataSet
        Try
            ...
            'objDA.strQuery += " U_Z_ApproveBy,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS U_Z_NextApprover,convert(varchar(10),CreateDate,103) as CreateDate ,LEFT(CONVERT(VARCHAR(5), CreateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), CreateTime, 9),2) AS CreateTime, convert(varchar(10),UpdateDate,103) as UpdateDate,LEFT(CONVERT(VARCHAR(5), UpdateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), UpdateTime, 9),2) AS UpdateTime,Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus,U_Z_Remarks From [@Z_HR_LOANHIS] T0 "
            'objDA.strQuery += " Where U_Z_DocType = '" + objEN.HistoryType + "'"
            'objDA.strQuery += " And U_Z_DocEntry = '" + objEN.DocEntry + "'"

            objDA.strQuery = " Select DocEntry,U_Z_DocEntry,U_Z_DocType,T1.ExtEmpNo AS U_Z_EmpId,U_Z_EmpName,U_Z_LoanCode,CAST(isnull(U_Z_LoanAmount,0) as decimal(10,2)) AS U_Z_LoanAmount,convert(varchar(10),U_Z_DisDate,103) as U_Z_DisDate,convert(varchar(10),U_Z_StartDate,103) as U_Z_StartDate,U_Z_NoEMI,"
            objDA.strQuery += " U_Z_ApproveBy,isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NextApprover ),'') AS U_Z_NextApprover,convert(varchar(10),CreateDate,103) as CreateDate ,LEFT(CONVERT(VARCHAR(5), CreateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), CreateTime, 9),2) AS CreateTime, convert(varchar(10),T0.UpdateDate,103) as UpdateDate,LEFT(CONVERT(VARCHAR(5), UpdateTime, 9),2) + ':' + RIGHT(CONVERT(VARCHAR(30), UpdateTime, 9),2) AS UpdateTime,Case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end AS U_Z_AppStatus,U_Z_Remarks From [@Z_HR_LOANHIS] T0 JOIN OHEM T1 ON T0.U_Z_EmpId=T1.empID"
            objDA.strQuery += " Where U_Z_DocType = '" + objEN.HistoryType + "'"
            objDA.strQuery += " And U_Z_DocEntry = '" + objEN.DocEntry + "'"
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
