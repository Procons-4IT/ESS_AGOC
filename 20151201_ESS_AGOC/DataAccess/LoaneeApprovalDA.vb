...
Public Class LoaneeApprovalDA
    ...
    Public Function MainGridBind(ByVal objEN As ExpClaimApprovalEN) As DataSet
        Try
            objDA.strQuery = " select distinct(T0.Code),T0.U_Z_TAEmpID,T0.U_Z_EmpID,T0.U_Z_EmpName,Convert(Varchar(10),T0.U_Z_SubDt,103) AS U_Z_SubDt,T0.U_Z_Client,T0.U_Z_Project,Case T0.U_Z_DocStatus when 'C' then 'Closed' else 'Opened' end as U_Z_DocStatus  from [@Z_HR_LOEXPCL] T0"
            objDA.strQuery += " Left outer Join [@Z_HR_LEXPCL] T1 on T0.Code=T1.U_Z_DocRefNo "
            ...
            objDA.strQuery += " And ( T1.U_Z_NxtApprover = '" + objEN.UserCode + "')"
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T1.U_Z_AppRequired,'N')='Y' and isnull(T0.U_Z_DocStatus,'O')='O' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'Loanee'  Order by T0.Code Desc;"
            ...
            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T4.DocEntry = T2.DocEntry "
            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry  "
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T1.U_Z_AppRequired,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'Loanee' Order by T0.Code Desc"
    ...
    Public Function ExpensesRequestApproval(ByVal objEN As ExpClaimApprovalEN) As DataSet
        Try
            objDA.strQuery = " Select T0.U_Z_DocRefNo,T0.Code,T0.U_Z_EmpID,T0.U_Z_EmpName,Convert(Varchar(10),T0.U_Z_SubDt,103) AS U_Z_SubDt,T0.U_Z_Client,T0.U_Z_Project,Convert(Varchar(10),U_Z_Claimdt,103) AS U_Z_Claimdt,U_Z_ExpType,U_Z_Currency,cast(U_Z_CurAmt as decimal(10,2)) AS ""U_Z_CurAmt"",cast(U_Z_ExcRate as decimal(10,2)) AS ""U_Z_ExcRate"",U_Z_UsdAmt,U_Z_ReimAmt,U_Z_Attachment,Isnull(T0.U_Z_AppStatus,'P') AS U_Z_AppStatus,Convert(Varchar(10),isnull(T0.""U_Z_Month"",MONTH(U_Z_Claimdt))) AS ""U_Z_Month"",Convert(Varchar(10),isnull(T0.""U_Z_Year"",YEAR(U_Z_Claimdt))) AS ""U_Z_Year"","
            objDA.strQuery += " T0.U_Z_RejRemark AS U_Z_Remarks,Case T0.U_Z_TripType when 'N' then 'New' else 'Existing' End as U_Z_TripType,T0.U_Z_Posting,T0.U_Z_TraDesc,U_Z_CurApprover,U_Z_NxtApprover,T1.U_Z_EmpID as 'TAEmpID', Case T0.U_Z_Reimburse when 'Y' then 'Yes' else 'No' End as  'U_Z_Reimburse', "
            ...
            objDA.strQuery += "  From [@Z_HR_LEXPCL] T0 "
            ...
            objDA.strQuery += " where ( T0.U_Z_NxtApprover = '" + objEN.UserCode + "')"
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  isnull(T0.U_Z_AppRequired,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'Loanee' and T0.U_Z_DocRefNo='" & objEN.DocEntry & "' and T0.U_Z_EmpID='" & objEN.EmpUserId & "' Order by T0.U_Z_Claimdt,Convert(Numeric,T0.Code) Desc;"
            objDA.strQuery += " Select T0.Code,T0.U_Z_EmpID,T0.U_Z_EmpName,Convert(Varchar(10),T0.U_Z_SubDt,103) AS U_Z_SubDt,T0.U_Z_Client,T0.U_Z_Project,Case T0.U_Z_TripType when 'N' then 'New' else 'Existing' End as U_Z_TripType,T0.U_Z_TraDesc,Case isnull(T0.U_Z_DocStatus,'O') when 'C' then 'Closed' else 'Opened' end as U_Z_DocStatus,U_Z_TAEmpID AS TAEmpID  from [@Z_HR_LOEXPCL] T0 where Code='" & objEN.DocEntry & "';"
            ...
            objDA.strQuery += " And T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'Loanee';"
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
    ...
    Public Function BindExpenseSummaryApproval(ByVal objEN As ExpClaimApprovalEN) As DataSet
        Try
            ...
            objDA.strQuery += " , isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS CurAPPName, "
            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS NextAppName"
            objDA.strQuery += " , U_Z_CurApprover,U_Z_NxtApprover From [@Z_HR_LEXPCL] T0 JOIN [@Z_HR_APPT1] T1 ON T0.U_Z_EmpID = T1.U_Z_OUser "
            objDA.strQuery += " JOIN [@Z_HR_APPT2] T2 ON T1.DocEntry = T2.DocEntry "
            objDA.strQuery += " JOIN [@Z_HR_OAPPT] T3 ON T2.DocEntry = T3.DocEntry  "
            objDA.strQuery += " And isnull(T2.U_Z_AMan,'N')='Y' AND isnull(T3.U_Z_Active,'N')='Y' and  T2.U_Z_AUser = '" + objEN.UserCode + "' And T3.U_Z_DocType = 'Loanee' where T0.U_Z_DocRefNo='" & objEN.DocEntry & "' and T0.U_Z_EmpID='" & objEN.EmpUserId & "' Order by U_Z_Claimdt,Convert(Numeric,Code) Desc"
            objDA.sqlda = New SqlDataAdapter(objDA.strQuery, objDA.con)
    ...
    Public Function LoadHistory(ByVal objEN As ExpClaimApprovalEN) As DataSet
        Try
            ...
            objDA.strQuery += " Where U_Z_DocType = 'Loanee'"
            objDA.strQuery += " And U_Z_DocEntry = '" + objEN.DocEntry + "'"
    ...
    Public Function addUpdateDocument(ByVal objEN As ExpClaimApprovalEN) As String
        Try
            ...
            objDA.strQuery += " where T0.U_Z_DocType='Loanee' AND T2.U_Z_OUser='" & objEN.EmpId & "' AND T1.U_Z_AUser='" & objEN.UserCode & "'"
            ...
            End If
            objDA.strQuery = "Select * from [@Z_HR_APHIS] where U_Z_DocEntry='" & objEN.DocEntry & "' and U_Z_DocType='Loanee' and U_Z_ApproveBy='" & objEN.UserCode & "'"
            oRecordSet.DoQuery(objDA.strQuery)
    ...
    Public Function updateFinalStatus(ByVal objEN As ExpClaimApprovalEN) As String
        Try
            ...
            Dim StrMailMessage, strClaimDocNo As String
            ...
            oTemp.DoQuery("Select * from [@Z_HR_LEXPCL] where Code='" & objEN.DocEntry & "'")
            If oTemp.RecordCount > 0 Then
                strClaimDocNo = oTemp.Fields.Item("U_Z_DocRefNo").Value
            Else
                strClaimDocNo = objEN.DocEntry
            End If
            If objEN.AppStatus = "A" Then
                objDA.strQuery = " Select T2.DocEntry "
                objDA.strQuery += " From [@Z_HR_APPT2] T2 "
                ...
                oRecordSet.DoQuery(objDA.strQuery)
                If Not oRecordSet.EoF Then
                    objDA.strQuery = "Update [@Z_HR_LEXPCL] Set U_Z_Year=" & objEN.Year & ",U_Z_Month=" & objEN.Month & ", U_Z_AppStatus = 'A' Where Code = '" + objEN.DocEntry + "'"
                    oRecordSet.DoQuery(objDA.strQuery)
                    ...
                    If objEN.PostingType = "G/L Account" Or objEN.PostingType = "G" Then
                        objDA.strmsg = objBL.LoaneeCreateJournelVouchers(objEN.DocEntry, objEN.Reimbused, objEN.SapCompany)
                    End If
                    If objDA.strmsg = "Success" Then
                        StrMailMessage = "Expenses Claim request approved for the request number is :" & strMailDocEntry
                        StrMailMessage = "Expenses Claim request approved for the request code : " & strMailDocEntry & " in the expense claim number: " & Integer.Parse(strClaimDocNo)
                        'objBL.SendMail_RequestApproval(StrMailMessage, objEN.EmpId, objEN.SapCompany)
                    Else
                        objDA.strQuery = "Update [@Z_HR_LEXPCL] Set U_Z_AppStatus = 'P' Where Code = '" + objEN.DocEntry + "'"
                        oRecordSet.DoQuery(objDA.strQuery)
                    End If
                End If
            ElseIf objEN.AppStatus = "R" Then
                ...
                oRecordSet.DoQuery(objDA.strQuery)
                If Not oRecordSet.EoF Then
                    objDA.strQuery = "Update [@Z_HR_LEXPCL] Set U_Z_AppStatus = 'R',U_Z_RejRemark='" & objEN.Remarks & "' Where Code = '" + objEN.DocEntry + "'"
                    oRecordSet.DoQuery(objDA.strQuery)
                    ' StrMailMessage = "Expenses Claim request rejected for the request number is :" & CInt(objEN.DocEntry)
                    StrMailMessage = "Expenses Claim request rejected for the request code : " & objEN.DocEntry & " in the expense claim number: " & Integer.Parse(strClaimDocNo)
                    objBL.SendMail_RequestApproval(StrMailMessage, objEN.EmpId, objEN.SapCompany)
    ...
    Public Function UpdateDocStatus(ByVal objEN As ExpClaimApprovalEN) As String
        Try
            ...
            oRecordSet.DoQuery(objDA.strQuery)
            If Not oRecordSet.EoF Then
                objDA.strQuery = "Update [@Z_HR_LOEXPCL] set U_Z_DocStatus='" & objEN.AppStatus & "' where Code='" & objEN.HeadDocEntry & "'"
                oRecordSet.DoQuery(objDA.strQuery)
                objDA.strmsg = "Success"
            End If
            Return objDA.strmsg
    ...
    Public Sub SendMessage(ByVal strReqNo As String, ByVal strEmpNo As String, ByVal strAuthorizer As String, ByVal MailDocEntry As String, ByVal objCompany As SAPbobsCOM.Company, ByVal strEmpName As String)
        Try
            ...
            strQuery += " where T0.U_Z_DocType='Loanee' AND T2.U_Z_OUser='" & strEmpNo & "' AND T1.U_Z_AUser='" & strAuthorizer & "'"
            ...
            If Not oRecordSet.EoF Then
                ...
                If Not oRecordSet.EoF Then
                    ...
                    strQuery = "Update [@Z_HR_LEXPCL] set U_Z_CurApprover='" & strAuthorizer & "',U_Z_NxtApprover='" & strMessageUser & "' where Code in (" & MailDocEntry & ")"
                    oTemp.DoQuery(strQuery)
