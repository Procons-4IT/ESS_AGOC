...
Public Class NewTrainingDA
    ...
    Public Function BindNewTraining(ByVal objEN As NewTrainingEN) As DataSet
        Try
            ...
            objDA.strQuery += " case U_Z_AppStatus when 'P' then 'Pending' when 'A' then 'Approved' when 'R' then 'Rejected' end as U_Z_AppStatus,U_Z_CrEmpName,"
            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_CurApprover ),'') AS ""U_Z_CurApprover"","
            objDA.strQuery += " isnull((select T13.firstName +' '+ISNULL(T13.middleName,'') +' '+ isnull(T13.lastName,'')  from OHEM T13 JOIN OUSR T14 ON T14.INTERNAL_K =T13.userId where T14.USER_CODE=T0.U_Z_NxtApprover ),'') AS ""U_Z_NxtApprover""  from [@Z_HR_ONTREQ] T0 where U_Z_HREmpID='" & objEN.EmpId & "' Order by DocEntry Desc"
    ...
    Public Function PopulateEmployee(ByVal objen As NewTrainingEN) As NewTrainingEN
        Try
            objen.StrQry = "Select isnull(ExtEmpNo,'') as 'Badge',dept,empID,isnull(firstName,'') +' '+ isnull(middleName,'') +' '+ isnull(lastName,'') as 'EmpName', isnull(position,0) as position,T1.descriptio  from OHEM T0 left join OHPS T1 on T0.position=t1.posID  where empID=" & objen.EmpId & ""
            objDA.sqlda = New SqlDataAdapter(objen.StrQry, objDA.con)
            objDA.sqlda.Fill(objDA.ds2)
            If objDA.ds2.Tables(0).Rows.Count > 0 Then
                objen.Badge = objDA.ds2.Tables(0).Rows(0)("Badge").ToString()
    ...
    Public Function SaveNewTrainingRequest(ByVal objen As NewTrainingEN) As String
        Try
            ...
            oGeneralData1.SetProperty("U_Z_CrEmpID", objen.AltRequestEmpId)
            oGeneralData1.SetProperty("U_Z_CrEmpName", objen.AltRequestEmpName)
            If objen.ReqCode = "" Then
