...
Public Class SeniorMgrAppraisalDA
    ...
    Public Function PageLoadBind(ByVal objen As SeniorMgrAppraisalEN) As DataSet
        Try
            ...
            objDA.strQuery += " then 'Closed' else 'HR Canceled' end as U_Z_Status,case U_Z_WStatus when 'DR' then 'Draft' when 'HR' then 'HR Approved' when 'SM' then "
            objDA.strQuery += " 'Sr.Manager Approved' when 'LM' then 'LineManager Approved' when 'SE' then 'SelfApproved' else 'HR Canceled' end as 'U_Z_WStatus'   "
    ...
    Public Sub UpdatesrMgrAppBusiness(ByVal objen As SeniorMgrAppraisalEN)
        Try
            objen.StrQry = "Update ""@Z_HR_SEAPP1"" set ""U_Z_SMRaCode""='" & objen.SelfRating & "',""U_Z_BussSMRate""=" & objen.Amount & ",U_Z_SrRemark='" & objen.SecondRemarks & "',U_Z_BussSMGrade='" & objen.BussSelfGrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "' and ""LineId""=" & objen.LineNo & ""
            objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
            objDA.con.Open()
            objDA.cmd.ExecuteNonQuery()
            objDA.con.Close()
    ...
    Public Sub UpdatesrMgrAppPeople(ByVal objen As SeniorMgrAppraisalEN)
        Try
            objen.StrQry = "Update ""@Z_HR_SEAPP2"" set ""U_Z_SMRaCode""='" & objen.SelfRating & "', ""U_Z_PeoSMRate""=" & objen.Amount & ",U_Z_SrRemark='" & objen.SecondRemarks & "',U_Z_PeoSMGrade='" & objen.PeoSelfGrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "' and ""LineId""=" & objen.LineNo & ""
            objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
            objDA.con.Open()
            objDA.cmd.ExecuteNonQuery()
            objDA.con.Close()
    ...
    Public Sub UpdatesrMgrAppCompetence(ByVal objen As SeniorMgrAppraisalEN)
        Try
            objen.StrQry = "Update ""@Z_HR_SEAPP3"" set ""U_Z_SMRaCode""='" & objen.SelfRating & "',""U_Z_CompSMRate""=" & objen.Amount & ",U_Z_SrRemark='" & objen.SecondRemarks & "',U_Z_CompSMGrade='" & objen.CompSelfGrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "' and ""LineId""=" & objen.LineNo & ""
            objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
            objDA.con.Open()
            objDA.cmd.ExecuteNonQuery()
            objDA.con.Close()
        ...
        End Try
    End Sub
    Public Sub UpdateLineMgrAppGrade(ByVal objen As SeniorMgrAppraisalEN)
        Dim status, strgrade As String
        Try
            objDA.con.Open()
            objDA.cmd = New SqlCommand("Select COALESCE(SUM (U_Z_BussSMRate), 0)  from [@Z_HR_SEAPP1] where DocEntry=" & objen.AppraisalNumber & "", objDA.con)
            ...
            If CDbl(status) > 0 And status <> "" Then
                ...
                objen.StrQry = "Update ""@Z_HR_SEAPP1"" set ""U_Z_BusSMTotGrade""='" & strgrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "'"
                objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
                objDA.con.Open()
                objDA.cmd.ExecuteNonQuery()
                objDA.con.Close()
            End If
            objDA.con.Open()
            objDA.cmd = New SqlCommand("Select COALESCE(SUM (U_Z_PeoSMRate), 0) from [@Z_HR_SEAPP2] where DocEntry=" & objen.AppraisalNumber & "", objDA.con)
            objDA.cmd.CommandType = CommandType.Text
            status = objDA.cmd.ExecuteScalar()
            objDA.con.Close()
            If CDbl(status) > 0 And status <> "" Then
                strgrade = objDA.GetAppraisalGrade(status)
                objen.StrQry = "Update ""@Z_HR_SEAPP2"" set ""U_Z_PeoSMTotGrade""='" & strgrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "'"
                objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
                objDA.con.Open()
                objDA.cmd.ExecuteNonQuery()
                objDA.con.Close()
            End If
            objDA.con.Open()
            objDA.cmd = New SqlCommand("Select COALESCE(SUM (U_Z_CompSMRate), 0)  from [@Z_HR_SEAPP3] where DocEntry=" & objen.AppraisalNumber & "", objDA.con)
            objDA.cmd.CommandType = CommandType.Text
            status = objDA.cmd.ExecuteScalar()
            objDA.con.Close()
            If CDbl(status) > 0 And status <> "" Then
                strgrade = objDA.GetAppraisalGrade(status)
                objen.StrQry = "Update ""@Z_HR_SEAPP3"" set ""U_Z_CoSMTotGrade""='" & strgrade & "' where ""DocEntry""='" & objen.AppraisalNumber & "'"
                objDA.cmd = New SqlCommand(objen.StrQry, objDA.con)
                objDA.con.Open()
                objDA.cmd.ExecuteNonQuery()
                objDA.con.Close()
