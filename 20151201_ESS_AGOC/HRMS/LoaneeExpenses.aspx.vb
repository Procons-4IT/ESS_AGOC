...
Public Class LoaneeExpenses
    Inherits System.Web.UI.Page
    Dim objEN As ClaimRequestEN = New ClaimRequestEN()
    Dim objBL As LoaneeExpenseBL = New LoaneeExpenseBL()
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Dim strEmpType1 As String = objDA.GetLoaneeAppTemp(objEN.EmpId)
            If strEmpType1 = "" Then
    ...
    Protected Sub Btncallpop_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles Btncallpop.ServerClick
        Dim str1, str2, str3, strExpType, strPerdim, UserCode As String
        ...
        If txthidoption.Text = "Expenses" Then
            If txtpoptno.Text.Trim() <> "" Then
                ...
                If dbcon.ds4.Tables(0).Rows.Count > 0 Then
                    ...
                    strExpType = dbcon.ds4.Tables(0).Rows(0)("U_Z_ExpType").ToString()
                    strPerdim = dbcon.ds4.Tables(0).Rows(0)("U_Z_Perdim").ToString()
                    If strExpType = "F" Then
                        ddltranscur.Enabled = False
                        txttrasamt.Enabled = False
                        txtexrate.Enabled = False
                    Else
                        ddltranscur.Enabled = True
                        txttrasamt.Enabled = True
                        txtexrate.Enabled = False
                    End If
                    If strPerdim = "I" Then
                        txttratodt.Enabled = False
                    Else
                        txttratodt.Enabled = True
                    End If
        ...
        ElseIf txthidoption.Text = "AltEmp" Then
            If txtpoptno.Text.Trim() <> "" Then
                Dim strEmpType1 As String = objDA.GetLoaneeAppTemp(txtpopunique.Text.Trim())
                If strEmpType1 = "" Then
                    lblempNo.Text = txtpopunique.Text.Trim()
                    lblempname.Text = txtpoptno.Text.Trim()
                    lbtnNewExpenses.Visible = False
                    btnSubmit.Visible = False
                    lblNewTrip.Text = "Approval template is not defined for Employee : " & txtpoptno.Text.Trim() & "  . You can't create request. please Contact the Administrator..."
    ...
    Private Sub btnnew_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnnew.Click
        Try
            ...
            dbcon.dss5 = dbcon.AlternateRequest(Session("UserCode").ToString(), "Loanee")
    ...
    Private Sub btnClose_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnClose.Click
        ...
        oRecSet.DoQuery("Update ""@Z_HR_LEXPCL"" set ""Name""=""Code"" where U_Z_DocRefNo='" & lbldocno.Text.Trim() & "' and ""Name"" Like '%D'")
        panelview.Visible = True
        PanelNewRequest.Visible = False
    End Sub
    ...
    Public Function Validation() As Boolean
        Dim TranstoDate As Date
        Dim strHolidays, strweekdays As String
        ...
        Try
            ...
            ElseIf lblOverLap.Text = "Y" Then
                ...
                TranstoDate = Date.ParseExact(txttratodt.Text.Trim().Replace("-", "/").Replace(".", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture)
                Do While TransDate <= TranstoDate
                    ...
                    '    dbcon.strmsg = "You have an BTA for this date " & TransDate.ToString("yyyy-MM-dd") & ", you cannot proceed with another expense claim."
                    '    mess(dbcon.strmsg)
                    '    Return False
                    'End If
            ...
            TransDate = Date.ParseExact(txttrasdt.Text.Trim().Replace("-", "/").Replace(".", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture)
            TranstoDate = Date.ParseExact(txttratodt.Text.Trim().Replace("-", "/").Replace(".", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture)
            strHolidays = objBL.GetHolidayCode(lblempNo.Text.Trim())
            If TranstoDate = TransDate Then
                strweekdays = objBL.GetWeekdays(strHolidays, TransDate, dbcon.objMainCompany)
                If strweekdays <> "N" Then
                    Dim strsql As String
                    strsql = "Select isnull(U_Z_WeekEnd,'N') from ""@Z_HR_LEXPANCES"" where ""U_Z_ExpName""='" & txtexptype.Text & "'"
                    Dim Otest As SAPbobsCOM.Recordset
                    Otest = dbcon.objMainCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                    Otest.DoQuery(strsql)
                    If Otest.Fields.Item(0).Value = "Y" Then
                        dbcon.strmsg = "Transaction date is comes from weekdays/Holidays.You cannot apply the expenses for this date."
                        mess(dbcon.strmsg)
                        Return False
                    Else
                        strweekdays = "N"
            ...
            End If
            Do While TransDate <= TranstoDate
                blnflag = objBL.ValidateExpenses(txtexpcode.Text.Trim(), TransDate, lblempNo.Text.Trim(), dbcon.objMainCompany)
                If blnflag = True Then
                    dbcon.strmsg = "Expenses type and transaction date already applied.Transaction Date is " & TransDate & " and Expenses is " & txtexptype.Text.Trim()
                    mess(dbcon.strmsg)
                    Return False
                End If
                TransDate = TransDate.AddDays(1)
    ...
    Private Function SaveTempTable() As String
        ...
        Try
            ...
            Else
                TransDate = Now.Date
            End If
            If txttratodt.Text <> "" Then
                TranstoDate = Date.ParseExact(txttratodt.Text.Trim().Replace("-", "/").Replace(".", "/"), "dd/MM/yyyy", CultureInfo.InvariantCulture)
            Else
                TranstoDate = Now.Date
            End If
            ...
            Do While TransDate <= TranstoDate
                ...
                If strweekdays <> "N" Then
                    ...
                    End If
                End If
                dbcon.strQuery = "Insert Into [U_LEXPCLAIM] (U_SessionId,U_TANo,U_Empid,U_Empname,U_SubDate,U_Client,U_Project,U_ClimDate,U_TripType,U_TraCode,"
                ...
                dbcon.strQuery += " U_Notes,U_AppStatus,U_Attachment,U_Year,U_Month,U_DocRefNo,U_DebitCode,U_CreditCode,U_Posting,U_CardCode,U_Dimension,U_CrEmpName,U_Holidays) Values ('" & Session("SessionId").ToString() & "',"
                ...
                dbcon.strQuery += " " & TransDate.Year & "," & TransDate.Month & ",'" & txtreqcode.Text.Trim() & "','" & txtdebit.Text.Trim() & "',"
                dbcon.strQuery += " '" & txtCredit.Text.Trim() & "','" & txtposting.Text.Trim() & "','" & lblCardCode.Text.Trim() & "','" & Dimension & "',"
                dbcon.strQuery += " '" & Session("UserName").ToString() & "','" & strweekdays.Trim() & "') "
    ...
    Private Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        ElseIf Validation1() = True Then
            objEN.EmpId = lblempNo.Text.Trim() 'Session("UserCode").ToString()
            objEN.SessionID = Session("SessionId").ToString()
    ...
    Private Function SaveUpdateClaim(ByVal objEN As ClaimRequestEN) As String
        Try
            ...
            oUserTable1 = dbcon.objMainCompany.UserTables.Item("Z_HR_LOEXPCL")
            objEN.DocEntry = lbldocno.Text.Trim()
            If oUserTable1.GetByKey(objEN.DocEntry) Then
                ...
                End If
            Else
                objEN.DocEntry = dbcon.Getmaxcode("[@Z_HR_LOEXPCL]", "Code")
                oUserTable1.Code = objEN.DocEntry
                oUserTable1.Name = objEN.DocEntry
            ...
            dbcon.strQuery = "Select * from ""U_LEXPCLAIM""   where ""U_SessionId""='" & objEN.SessionID & "' AND ""U_Empid""='" & objEN.EmpId & "' and ""U_Holidays""='N'"
            oRecSet.DoQuery(dbcon.strQuery)
            If oRecSet.RecordCount > 0 Then
                oUserTable2 = dbcon.objMainCompany.UserTables.Item("Z_HR_LEXPCL")
                For introw As Integer = 0 To oRecSet.RecordCount - 1
                    'objEN.DocEntry = lbldocno.Text.Trim()
                    LineDocEntry = oRecSet.Fields.Item("U_Code").Value
                    If LineDocEntry = "" Then
                        LineDocEntry = dbcon.Getmaxcode("[@Z_HR_LEXPCL]", "Code")
                    End If
                    If oUserTable2.GetByKey(LineDocEntry) Then
                        ...
                        oUserTable2.UserFields.Fields.Item("U_Z_OverLap").Value = dbcon.GetOverlap(oRecSet.Fields.Item("U_ExpCode").Value.ToString(), "Loanee")
                        ...
                        Else
                            ...
                            If oRecSet.Fields.Item("U_Posting").Value = "G" Then
                                dbcon.strmsg = objDA.LoaneeCreateJournelVouchers(LineDocEntry, oRecSet.Fields.Item("U_ReImbused").Value, dbcon.objMainCompany)
                            End If
                            dbcon.strmsg = "Success"
                        End If
                    Else
                        ...
                        oUserTable2.UserFields.Fields.Item("U_Z_Year").Value = oRecSet.Fields.Item("U_Year").Value
                        oUserTable2.UserFields.Fields.Item("U_Z_Month").Value = oRecSet.Fields.Item("U_Month").Value
                        oUserTable2.UserFields.Fields.Item("U_Z_AppStatus").Value = dbcon.DocApproval("Loanee", lblempNo.Text.Trim())
                        oUserTable2.UserFields.Fields.Item("U_Z_DocRefNo").Value = objEN.DocEntry
                        oUserTable2.UserFields.Fields.Item("U_Z_CardCode").Value = lblCardCode.Text.Trim()
                        oUserTable2.UserFields.Fields.Item("U_Z_DebitCode").Value = oRecSet.Fields.Item("U_DebitCode").Value
                        oUserTable2.UserFields.Fields.Item("U_Z_CreditCode").Value = oRecSet.Fields.Item("U_CreditCode").Value
                        ...
                        Else
                            ...
                            If oRecSet.Fields.Item("U_Posting").Value = "G" Then
                                objDA.LoaneeCreateJournelVouchers(LineDocEntry, oRecSet.Fields.Item("U_ReImbused").Value, dbcon.objMainCompany)
                            End If
                            intTempID = dbcon.GetTemplateID("Loanee", lblempNo.Text.Trim())
                            If intTempID <> "0" Then
                                dbcon.UpdateApprovalRequired("@Z_HR_LEXPCL", "Code", LineDocEntry, "Y", intTempID)
                            Else
                                dbcon.UpdateApprovalRequired("@Z_HR_LEXPCL", "Code", LineDocEntry, "N", intTempID)
                            End If
                            dbcon.strmsg = "Success"
                        End If
                    End If
                ...
                Next
            End If
            dbcon.strQuery = "Delete from  ""@Z_HR_LEXPCL""  where ""U_Z_DocRefNo""='" & objEN.DocEntry & "' and ""Name"" Like '%D'"
            oRecSet.DoQuery(dbcon.strQuery)
            If strMailCode <> "" Then
                dbcon.InitialMessage("Loanee Expenses Claim", objEN.DocEntry, dbcon.DocApproval("Loanee", lblempNo.Text.Trim()), intTempID, lblempname.Text.Trim(), "Loanee", dbcon.objMainCompany, strMailCode)
            End If
            dbcon.strmsg = "Success"
        Catch ex As Exception
            dbcon.strmsg = ex.Message
    ...
    Private Sub grdRequestExpenses_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles grdRequestExpenses.RowDeleting
        Try
            ...
            Blflag = dbcon.WithDrawStatus("Loanee", objEN.TravelCode)
    ...
    Private Sub grdApproved_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grdApproved.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Blflag = dbcon.WithDrawStatus("Loanee", LiDocNo.Text.Trim())
            If Blflag = True Then
                Liview.Visible = True
            Else
    ...
    Protected Sub lbtAppHistory_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            Else
                ...
                LoadHistory(DocNo.Text.Trim(), "Loanee")
    ...
    Private Sub txttrasdt_TextChanged(sender As Object, e As System.EventArgs) Handles txttrasdt.TextChanged
        txttratodt.Text = txttrasdt.Text.Trim()
        ModalPopupExtender6.Show()
    End Sub
End Class